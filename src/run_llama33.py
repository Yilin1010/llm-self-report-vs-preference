"""Llama-3.3-70B full replication: generation + per-turn u + post-conversation comparison, all via OpenRouter.

Differences from the main model: top-20 logprobs (not the full vocabulary); provider Novita.
`captured` is logged per turn for verification.
"""
...
OUT = ROOT / "results" / "transcripts_llama33"
CMP = ROOT / "results" / "comparisons_llama33.json"
KEY = [l.split("=",1)[1].strip() for l in (ROOT/".env").read_text().splitlines()
       if l.startswith("OPENROUTER_API_KEY=")][0]
MODEL = "meta-llama/llama-3.3-70b-instruct"
PROV = {"only": ["Novita"]}      # the only provider that returns logprobs
...
def gen(history, seed):
    d = call({"model": MODEL, "provider": PROV,
...
def digits(history):
    ...
    for e in top:
        ...
        if len(t) == 1 and t in DIGITS:
            p[t] = p.get(t, 0.0) + math.exp(e["logprob"])
    tot = sum(p.values())
    if tot <= 0:
        return None, 0.0
    return sum(int(k)*v for k, v in p.items())/tot, tot
...
def main():
    ...
    for eid in ids:
        ...
        json.dump({"id":eid,"model":MODEL,"provider":"Novita","turns":turns},
    ...
    for v in "abc":
        for x,y in (("L","Lp"),("L","S"),("Lp","S"),("SN","S"),("L","SN")):
            ...
            f_, cf = ab(eu_prompt.build(ra, rb))
            r_, cr = ab(eu_prompt.build(rb, ra))
            if f_ is None or r_ is None:
                print("SKIP", a, b, flush=True); continue
            r_ = 1.0 - r_
            rec = {"a":a,"b":b,"tag":"%s_vs_%s"%(x,y),"model":MODEL,
                   "p_forward":f_,"p_reverse":r_,"p_a_preferred":(f_+r_)/2,
                   "position_bias":f_-r_,"captured":min(cf,cr)}
            res.append(rec)
